...
import { projectPath, repoLink, type Project } from '../../data/projects'
...
function renderWordSpans(text: string | undefined, gameKey: string) {
  if (!text) return null
  ...
  return parts.map((part, i) => {
    ...
    return (
      <span key={i} data-game-word={gameKey} className="game-word">
        {part}
      </span>
    )
  })
...
const BUTTON_CLASS = 'relative inline-flex cursor-pointer items-center justify-center overflow-hidden rounded-xl text-center font-medium px-3 py-1.5 text-[13px] lg:px-5 lg:py-2.5 lg:text-base'

interface ProjectStackCardProps {
  project: Project
  /** 1-based position in the homepage stack, for analytics. */
  position: number
  cardHeight?: string
...
export function ProjectStackCard({
  ...
  const description = project.cardDescription ?? project.summary
  const repo = repoLink(project)
  const detailHref = projectPath(project.slug)
  ...
  }, [description, project.tagline, project.tech])
  ...
  return (
    <div
      ...
      style={{ perspective: '1200px', position: 'relative', width: '100%', '--flip-safari-card-h': cardHeight } as CSSProperties}
      onMouseEnter={() => {
    ...
    >
      <div
        ...
        style={{
          ...
          willChange: cardHovered ? 'transform' : 'auto',
        }}
      >
        <div
          style={{
        ...
        >
          ...
          <div ref={leftColRef} data-card-leftcol className="flex flex-col justify-between lg:justify-center px-4 pt-8 pb-4 lg:px-10 lg:py-8" style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
            ...
            >
              {renderWordSpans(description, project.slug)}
            ...
            <div className="flex justify-start gap-2.5 lg:mt-[18px] items-center">
              {repo && (
                <a
                  ...
                  onClick={() => posthog?.capture('project_repo_opened', { project_name: project.title, slug: project.slug, url: repo.href })}
                >
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    <span>{repo.kind === 'repo' ? 'Repository' : 'Visit site'}</span>
                  </span>
                </a>
              )}
              <a
                ...
                onMouseEnter={(e) => {
                  warmDocument(detailHref)
                  setViewHovered(true)
                  e.currentTarget.style.backgroundColor = '#055a84'
                }}
                onFocus={() => warmDocument(detailHref)}
                onTouchStart={() => warmDocument(detailHref)}
                onMouseLeave={(e) => {
                  setViewHovered(false)
                  e.currentTarget.style.backgroundColor = '#0671A4'
                }}
                onClick={() => posthog?.capture('project_card_opened', { slug: project.slug, source: 'home_stack', position })}
              >
                <span className="inline-flex items-center gap-1.5">
          ...
          <div
            ...
            style={{
              ...
              backgroundColor: project.brand.color,
            }}
          >
            {enableBackground && project.brand.bgImage && (
              <>
                <img
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(180deg, ${project.brand.color}73 0%, ${project.brand.color}D9 55%, ${project.brand.color}FF 100%)`,
                  }}
                />
              </>
            )}
            <div
              ...
              style={{
                ...
                isolation: 'auto',
                mixBlendMode: project.logo?.blend ?? 'screen',
              }}
            >
              <ProjectLogo project={project} variant="card" />
            </div>
          </div>
        </div>
      </div>
