...
export const ContainerScroll: React.FC<
  React.HTMLAttributes<HTMLDivElement> & { scrollYProgress?: MotionValue<number> }
> = ({ children, style, className, scrollYProgress: externalProgress, ...props }) => {
  const scrollRef = React.useRef<HTMLDivElement>(null)
  const { scrollYProgress: internalProgress } = useScroll({
    target: externalProgress ? undefined : scrollRef,
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={externalProgress ? undefined : scrollRef}
        className={cn(externalProgress ? "relative w-full h-full" : "relative min-h-svh w-full", className)}
...
>(
  (
    ...
    const y = useTransform(scrollYProgress, range, ["0%", isLastCard ? "0%" : "-350%"])
    ...
    const filter =
      variant === "light" || variant === undefined
